...
GOOS=linux GOARCH=arm go build -o build/threshold

cd build
tar -czvf threshold-arm.tar.gz threshold
cd ../

FILE_PATH="threshold-arm.tar.gz"
FILE_NAME="build/threshold-arm.tar.gz"
